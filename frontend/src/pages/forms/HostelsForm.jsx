...
export function HostelsForm(){
  const [ht,setHt]=useState(''); const [name,setName]=useState(''); const [room,setRoom]=useState(''); const [amount,setAmount]=useState('');
  ...
  async function submit(e){ e.preventDefault(); if(!isValidHTNo(ht)) return alert('Invalid HT'); const res = await initiatePayment({ student_roll:ht, student_name:name, amount:Number(amount), payment_type:'HOSTEL', payment_subtype:'ROOM' , remarks:`Room:${room}` }); setPaymentData(res); }
  return (
    <div>
